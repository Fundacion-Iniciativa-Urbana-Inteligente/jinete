...
app.post('/api/unlock', async (req, res) => {
  try {
    ...
    if (!expirationTime || Date.now() > expirationTime) {
      await tokenRef.delete(); // 🔥 Elimina el token inmediatamente si está expirado
    ...
    // 🔹 Normaliza `bikeId` para evitar errores de coincidencia
    const normalizedBikeId = bikeId.toString().trim().toLowerCase();

    // 🔹 Localizar en el array la bici con `bike_id === normalizedBikeId`
    ...
    await newRideRef.set({
      ...
      userId: userId,
      bikeId: normalizedBikeId,  // 🔹 Guardamos el `bikeId` corregido
      status: 'iniciado',
      startTime: new Date().toISOString(),
      startLat: startLat, // 🔹 Asegura que `startLat` no sea undefined
      startLon: startLon  // 🔹 Asegura que `startLon` no sea undefined
    });

    console.log(`✅ Viaje iniciado para bicicleta ${normalizedBikeId}.`);

    // 🔥 Eliminar el token después de su uso para evitar reutilización
...
app.post("/webhook", async (req, res) => {
  ...
  if (match) {
    ...
    let imei = bikeObj.imei; // 📌 Extraemos el IMEI si existe en `free_bike_status`
  
    // 🔹 Si el IMEI no está en `free_bike_status`, buscamos en `deviceLocations` usando `deviceName`
    if (!imei) {
      console.log(`🔍 Buscando IMEI en 'deviceLocations' para la bicicleta: ${bikeName}`);
  
      const deviceQuery = db.collection("deviceLocations").where("deviceName", "==", bikeObj.bike_id).limit(1);
      const deviceSnap = await deviceQuery.get();
  
      if (!deviceSnap.empty) {
        const deviceDoc = deviceSnap.docs[0];
        imei = deviceDoc.data().imei;
      }
    }
  
    // 🔹 Si sigue sin encontrarse el IMEI, mostramos un mensaje de error
    if (!imei) {
      await sendMessage(
        `⚠️ La bicicleta "${bikeName}" no tiene un IMEI registrado en el sistema. Contacta soporte.`,
        From
      );
      return res.status(200).send("Error: IMEI no encontrado");
    }
  
    // 🔹 Guardamos la bici en la sesión del usuario
    const sessionRef = db.collection("users_session").doc(From);
    await sessionRef.set(
      {
        step: "request_dni",
        selectedBikeImei: imei, // Ahora tenemos el IMEI correcto
        selectedBikeName: bikeObj.bike_id || "Desconocido",
      },
      { merge: true }
    );
